...
$md = file_get_contents(__DIR__ . "/../ConcursoMataNazis.md");
...
file_put_contents(__DIR__ . "/../AccConcursoMataNazis.md", str_replace(["\sp", "\sc", "\sinc", "\conc", "&nbsp;\n", "\n\n\n"], "", $md));
...
file_put_contents(__DIR__ . "/../ConcursoMataNazis.html", $html);

/* Generamos Metas */
/* -------------------------------------------------------------- */
$metas = "InfoKey: Subject\n";
$metas .= "InfoValue: ".$tags['DESCRIPTION']."\n\n";
...
$doc->loadHTMLFile(__DIR__ . "/../ConcursoMataNazis.html");
...
$metas .= bookMark("Licencia de uso", 1, 2);
$json[] = ["title" => "Licencia de uso","page" => 2];
$metas .= bookMark("Índice", 1, 3);
$json[] = ["title" => "Índice","page" => 3];
...
foreach ($json as $item) {
  if(isset($item['tag']) && in_array($item['tag'], ['H1', 'H2', 'H3'])) $indice .= '<a href="#anchor' . $item['page']-1 . '" class="like' . $item['tag'] . '"><span>' . $item['page'] . '</span>' . $item['title'] . '</a>';
}
